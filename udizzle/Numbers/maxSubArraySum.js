...
let maxSubArray_Iterative = function(arr) {
    if (arr.length === 1) {
        return arr[0];
    }
    let subArr = [];
...
let maxSubArray = function(arr, iteration) {
    if (!iteration) {
        iteration = 0;
    }
    ...
    return maxSubArray(arr.slice(++iteration, arr.length));
};
...
console.log(JSON.stringify(maxSubArray(IDEAL_INPUT, 0)));
